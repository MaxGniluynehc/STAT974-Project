...
if __name__ == '__main__':
    tc.random.manual_seed(21020403210)

    Hin, Hout, rnn_type, device, garch_type, epochs, lr, batch_size, bws = \
   (logr_df.shape[-1]+4, 2, "lstm_whole", "cpu", "GJR-EXP-EWMA", 10, 1e-3, 128, 21)
    ...
    logs = tc.empty(0)
    # logs = tc.tensor(np.loadtxt(logs_PATH+"logs_to_epoch={}.txt".format())).to(dtype=tc.float32, device="cpu")
    ...
    for epoch in tqdm(range(0, 0+epochs)):
        ...
        for idx, db in enumerate(dataloader):
            ...
            # (h1, c1), (h2, c2), (h3, c3) = h_init
            # for i in range(db.shape[1]):
            #     h1, c1 = volpredictor.lstm1(db[:,i,:], (h1, c1)) # x[:,i,:]: [bs, Hin], h1: [bs, 10]
            #     h1 = volpredictor.drop1(h1)
            #     h2, c2 = volpredictor.lstm2(h1, (h2, c2)) # h2: [bs, 4]
            #     h2 = volpredictor.drop2(h2)
            #     h3, c3 = volpredictor.lstm3(h2, (h3, c3)) # h3: [bs, Hout]
            #     h3 = volpredictor.drop2(h3)
            #     # o[:,i,:] = h3
            # out = tc.nn.Sequential(volpredictor.fc1, volpredictor.sigmoid,
            #                  volpredictor.fc2  # , self.relu
            #                  )(h3)
